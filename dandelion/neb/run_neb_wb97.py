...
def process_seed(seed, n_images, neb_fmax, cineb_fmax, steps, output_path):
    ...
    with SuppressStderr(): # xTB is so noisy when not converged
        try:
            ...
            os.environ["ASE_ORCA_COMMAND"] = f"/home/mhyeok/package/orca/orca_5_0_4/orca PREFIX.inp > PREFIX.out"
            uu=uuid.uuid4()
            ...
            for i, atom_config in enumerate(atom_configs):
                atom_config.calc = ORCA(label=os.path.join(os.path.dirname(output_path), f"orca/{uu}/{i}"),
                                        orcasimpleinput="wB97X 6-31G(d) NoTrah",
                                        orcablocks='%PAL NPROCS 32 END')
